...
def get_token():
    ...
    try:
        ...
        print(f"❌ Error HTTP al obtener token: {e.code}")
...
def search_products(token, queries):
    ...
    try:
        req = urllib.request.Request(MODEL_ENDPOINT, data=data, method='POST')
        ...
        print(f"\n❌ Error HTTP en la búsqueda: {e.code}")
        print(e.read().decode('utf-8'))
        sys.exit(1)
    except Exception as e:
